...
title_lbl = Label(tops, font=("monospaced", 36, "bold"), text="Welcome to our bank", fg="LightBlue", bg="black", bd=10)
    ...
    try:
        ...
        try:
            ...
            if float(amount) <= 0:
                ...
                balance = new_dep_bal
                spacer3.config(text="New Balance: Rs." + str(balance), fg="LimeGreen")
        except ValueError:
            spacer3.config(text="Amount Entered is Invalid! Try Again", fg="red")
    except TypeError:
        spacer3.config(text="Amount Entered is Invalid! Try Again", fg="red")
...
def withdrawn():
    ...
    try:
        ...
        try:
            ...
            if float(wd) > float(balance):
                spacer4.config(text="Insufficient funds!", fg="red")

            elif float(wd) > float(max_limit):
                spacer4.configure(text="Maximum Withdrawal Limit Rs. 20,000", fg="red")

            elif (float(wd) % 100) != 0:
                spacer4.configure(text="Multiples of Rs.100 only! Try again", fg="red")
                ...
                balance = new_wd_bal
                spacer4.configure(text="New Balance: Rs." + str(balance), fg="LimeGreen")
...
def bal():
    ...
    spacer3.config(text="")
    spacer4.config(text="")
    spacer5.config(text="Total Balance: Rs." + str(balance))
...
def add_new_user():
    ...
    total_lbl.config(text="Rs." + dep_amt + " Added Successfully!", fg="LimeGreen")
